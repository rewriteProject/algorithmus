...
class statistics:
    ...
    def s1_products_in_timespan(country, min, max, type=""):
        ...
        # REST GET
        if type == '':
            url = 'http://' + str(chris_address) + ':' + str(chris_port) + '/analytics/statistics/{}/all'.format(country)
            params = {'minDate': min, 'maxDate': max}
            request = requests.post(url, params)
        else:
            url = 'http://' + str(chris_address) + ':' + str(chris_port) + '/analytics/statistics/{}/{}'.format(country, type)
            print(url)
            params = {'minDate': min, 'maxDate': max}
            request = requests.post(url, params)
